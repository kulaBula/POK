...
def algorytm_sa(rozwiazanie_poczatkowe, czasy_poczatkowe, l:int, max_iter:int, temperatura:float, wsp_chlodzenia:float, cmax:float):
    ...
    while(iteracje<max_iter):
        ...
        if(delta < 0):
            print(f"Nowe lepsze rozwiazanie: {new_c}, delta: {delta}")
            c = new_c
            x = sasiad 
            y = czasy_sasiada
...
FILE = "m25.txt"
zadania, liczba_procesorow = wczytywanie(FILE, True)
...
print("C_max alg zachlannego = ", c_max)
# 3. Poprawianie rowiązania bazowego algorytmem symulowanego wyżarzania
ITER = 50000
TEMP = 20000
ALFA = 0.9999999 # Współczynnik chłodzenia
